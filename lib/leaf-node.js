...
class LeafNode {
    ...
    getBuffer(opts = {}) {
        ...
        let br = this[SYMBOLS.PREFIX_BR] || opts.defaultBr;
        return Buffer.concat([br].concat(this[SYMBOLS.RAW_CONTENT].flatMap(row => [row.raw, row.br].filter(val => val))));
    }
