...
class MimeNode {
    ...
    constructor(contentType, contentData, opts) {
        ...
        // must be called after addresses have been processed
    ...
    generateMessageId() {
        let fromAddress = this.contentData.from?.obj?.[0]?.address;
        let fromDomain;
        if (fromAddress) {
            fromDomain = fromAddress.split('@').pop().trim();
        }
    ...
    prepareAddressValues(contentData) {
        ...
        for (let addrKey of ['from', 'to', 'cc', 'bcc', 'sender', 'replyTo']) {
            ...
            if (typeof contentData[addrKey] === 'string') {
                let addrValue = addressparser(contentData.from);
                if (addrValue && addrValue.length) {
                    this.contentData[addrKey] = {
                        obj: addrValue,
                        str: contentData[addrKey].trim()
                    };
                }
            } else {
                let objVal = []
    ...
    setContentTransferEncoding(encoding) {
        ...
        if (parsedContentTransferEncoding) {
            ...
            if (contentTransferEncoding !== this.contentTransferEncoding && this[SYMBOLS.RAW_CONTENT].len) {
                // TODO: convert from old encoding to new
            }
    ...
    updateAddresses() {
        for (let addrKey of ['from', 'to', 'cc', 'bcc', 'sender', 'replyTo']) {
            ...
            let addrHeaders = this.headers.get(headerKey);
    ...
    updateDate() {
        let dateHeaders = this.headers.get('Date');
    ...
    updateMessageId() {
        let messageIdHeaders = this.headers.get('Message-ID');
    ...
    async build() {
        if (!this.parent) {
            // root node, so add required mime headers
            ...
            let mimeVersionHeaders = this.headers.get('MIME-Version');
