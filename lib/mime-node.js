...
const parseDkimHeaders = require('mailauth/lib/parse-dkim-headers');

const { normalizeCharset } = require('libmime/lib/charset');
...
const UTF_STRING_REGEX = /^(utf[-_]8|(us-?)?ascii|7bit)$/gi;
...
class MimeNode {
    ...
    decodeStr(input, sourceCharset) {
        ...
        sourceCharset = normalizeCharset(sourceCharset || 'utf-8');

        if (!sourceCharset || UTF_STRING_REGEX.test(sourceCharset)) {
            output = input;
    ...
    encodeStr(input, targetCharset) {
        ...
        targetCharset = normalizeCharset(targetCharset || 'utf-8');

        if (!targetCharset || UTF_STRING_REGEX.test(targetCharset)) {
            output = input;
            if (typeof output === 'string') {
                output = Buffer.from(output);
            }
    ...
    /**
     * Convert node content to the requested character set
     * @param {String} targetCharset Character set to convert to
     */
    convertCharset(targetCharset) {
        // set charset label as provided, without normalization
        let targetCharsetO = targetCharset;

        targetCharset = normalizeCharset(targetCharset || 'utf-8');
        let existingCharset = normalizeCharset(this.contentData.charset || 'utf-8');

        if (targetCharset === existingCharset) {
            return;
        }
    ...
    pushRawContent(raw, br) {
        ...
        if (this[SYMBOLS.RAW_CONTENT].chunks.length > 1000) {
            let lastEntry = this[SYMBOLS.RAW_CONTENT].chunks.pop();
            this[SYMBOLS.RAW_CONTENT].len -= lastEntry.length;

            let joinedContent = Buffer.concat(this[SYMBOLS.RAW_CONTENT].chunks, this[SYMBOLS.RAW_CONTENT].len);

            this[SYMBOLS.RAW_CONTENT].chunks = [joinedContent, lastEntry];
            this[SYMBOLS.RAW_CONTENT].len = joinedContent.length + lastEntry.length;
        }
    }
    ...
    removeChild(childNode) {
        assert.ok(this.multipartType, 'Not a multipart node');
        assert.ok(childNode instanceof MimeNode, 'Not a Mime Node object');

        for (let i = this[SYMBOLS.CHILD_NODES].length - 1; i >= 0; i--) {
            let node = this[SYMBOLS.CHILD_NODES][i];

            if (node === childNode) {
                this[SYMBOLS.CHILD_NODES].splice(i, 1);
                if (i > 0) {
                    let prevNode = this[SYMBOLS.CHILD_NODES][i - 1];
                    if (prevNode instanceof LeafNode && prevNode.type === 'BOUNDARY') {
                        // remove boundary leaf as well
                        this[SYMBOLS.CHILD_NODES].splice(i - 1, 1);
                    }
                }
                childNode[SYMBOLS.PARENT_NODE] = null;
                return childNode;
            }
        }
    ...
    delete() {
        assert.ok(this[SYMBOLS.PARENT_NODE], 'Not a child node');
        return this[SYMBOLS.PARENT_NODE].removeChild(this);
    }
    ...
    set disposition(disposition) {
        this.contentData.disposition = (disposition && disposition.toLowerCase().trim()) || null;
    ...
    /**
     * Get a header value for a specific key
     * @param {String} key Header key to get
     * @returns {String|Object|Array} decoded header value or a list of values
     */
    getHeader(key) {
        let matchingLines = this.headerObject
            .get(key)
            ...
            .flatMap(value => {
                switch (key.trim().toLowerCase()) {
                    case 'subject': {
                        let decodedSubject = value;

                        try {
                            decodedSubject = libmime.decodeWords(value);
                        } catch (err) {
                            // ignore
                        }
                    ...
                    case 'date': {
                        ...
                        if (date && date.toString() !== 'Invalid Date') {
                            return { value: date, original: value };
                        }
                        return { original: value };
                    }

                    case 'received': {
                        return parseReceived(`Recieved: ${value}`);
                    }

                    case 'content-disposition':
                    case 'content-type': {
                        let decodedValue = libmime.parseHeaderValue(value);
                        if (decodedValue.params.name) {
                            try {
                                decodedValue.params.name = libmime.decodeWords(decodedValue.params.name);
                            } catch (err) {
                                // ignore
                            }
                        }
                        if (decodedValue.params.filename) {
                            try {
                                decodedValue.params.filename = libmime.decodeWords(decodedValue.params.filename);
                            } catch (err) {
                                // ignore
                            }
                        }
                        decodedValue.original = value;
                        return decodedValue;
                    }
                    ...
                    case 'bimi-selector': {
                        return parseDkimHeaders(`${key}: ${value}`);
                    }
                    ...
                    case 'sender':
                    case 'reply-to': {
                        let parsed = addressparser(value);
                        let walk = entries => {
                            for (let entry of entries) {
                                if (entry.name) {
                                    try {
                        ...
                        walk(parsed);
                        return { value: parsed, original: value };
                    }
    ...
    set charset(targetCharset) {
        this.convertCharset(targetCharset);
    }
    ...
    set from(value) {
        ...
        this.addHeader(`From: ${this.contentData.from?.str}`, 'end', true);
    }
    ...
    set to(value) {
        ...
        this.addHeader(`To: ${this.contentData.to?.str}`, 'end', true);
    }
    ...
    set cc(value) {
        ...
        this.addHeader(`Cc: ${this.contentData.cc?.str}`, 'end', true);
    }
    ...
    set bcc(value) {
        ...
        this.addHeader(`Bcc: ${this.contentData.bcc?.str}`, 'end', true);
    }
    ...
    set sender(value) {
        ...
        this.addHeader(`Sender: ${this.contentData.sender?.str}`, 'end', true);
    }
    ...
    set replyTo(value) {
        ...
        this.addHeader(`Reply-To: ${this.contentData.replyTo?.str}`, 'end', true);
    }
