...
class MimeNode {
    ...
    constructor(contentType, contentData, opts = {}) {
        ...
        if (contentData && contentData.encoding && !this.multipartType) {
    ...
    setContentType(contentType) {
        ...
        let headerValue = libmime.foldLines('Content-Type: ' + libmime.buildHeaderValue(parsedContentType).replace(/\r?\n/g, '\n'));
        ...
        this.headers.insert(headerLine);
    }
    ...
    setContentTransferEncoding(encoding) {
        ...
        let headerValue = libmime.foldLines('Content-Transfer-Encoding: ' + libmime.buildHeaderValue(parsedContentTransferEncoding).replace(/\r?\n/g, '\n'));
        let headerEntry = headerValue
            .split('\n')
            .filter(val => val)
            .map(val => ({ raw: Buffer.from(val), br: this.defaultBr }));
        ...
        this.headers.insert(headerLine, 'end');
    }
