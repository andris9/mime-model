...
class MimeParser {
    ...
    async parseLine(lineBuf, br, opts = {}) {
        ...
        if (
            ...
            if (boundary) {
                // NB! Remove last line break before boundary
                let prevNode = this.nodeList.length ? this.nodeList[this.nodeList.length - 1] : null;
                if (prevNode) {
                    prevNode.removeLastBr();
                }
                ...
                switch (boundary.type) {
                    case 'BOUNDARY_SEPARATOR': {
                        ...
                        this.nodeList.push(separatorLeaf);
